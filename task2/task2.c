...
float mean_std_dev2(float* data, float* std_dev, int len)
{
	float sum=0;
	float squared_sum=0;
	float mean;
	float squared_std_dev;

	for(int i=0;i<len;i++)
	{
		sum += data[i];
		squared_sum += pow(data[i],2);
	}
	mean = sum/len;
	squared_std_dev = squared_sum/(len-1) - pow(mean,2);
...
int main()
{
	...
	//Bonus
	mean = mean_std_dev2(data,&std_dev,32);
	...
	printf("\nStandard deviation = %f", std_dev); //std_dev for part2 and bonus
